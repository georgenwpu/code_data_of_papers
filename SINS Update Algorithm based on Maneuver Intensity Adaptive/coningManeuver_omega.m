...
clc; clear; close all;
glvs;
afa = 3*glv.deg;                % half-apex angle
f = [0.01, 0.1, 1, 10];  w = (2*pi).*f;             % frequency
...
for k=1:N
    ...
    for ii = ss:ss:length(wm)
        ...
        sigma_d1(jj,:) = [sigma_ii, jj*ts*ss];
        sigma_ii = norm(cross(wm(ii-ss+1, :), wm(ii-ss+4,:)));
        sigma_d2(jj,:) = [sigma_ii, jj*ts*ss];
        omega_t(jj,:) = [f(k), jj*ts*ss];
        jj =  jj+1;
    end
end

figure;  colororder({glv.Lcolor{1}, glv.Lcolor{4}}); mki = 10:10:200;
yyaxis left;
p(1)=xlogyEx(sigma_d1(:,2), sigma_d1(:,1),'-o', 1,mki); hold on;
p(2)=xlogyEx(sigma_d2(:,2), sigma_d2(:,1),'-^', 1,mki);
xylabelsEx('time / s','MIP / (rad^2/s^2)');
yyaxis right;
p(3)=xlogyEx(omega_t(:,2), omega_t(:,1),'-s', 4,mki); set(gca, 'yscale','log');
xylabelsEx('time / s','Frequency / (Hz)');
l=legend(p, '\sigma_1','\sigma_2','f');
...
opt_res = [err_phi(1:50,1); err_phi(51:100,4); err_phi(101:150,7); err_phi(151:200,10);];
figure;
